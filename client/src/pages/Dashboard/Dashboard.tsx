...
const Dashboard: React.FC = () => {
  ...
  const canSeeFinancialStats = useMemo(
    () =>
      ["mb hr", "mbs head", "system_admin", "mb financial"].includes(
        userRoleName
      ),
    [userRoleName]
  );
  ...
  const eventsByDate = useMemo(() => {
    ...
    if (canSeeApprovals) {
      pendingRequests.forEach((r) => {
        const dateStr = (r.approver?.approver_due_date || "")
          .toString()
          .slice(0, 10);
        if (!dateStr) return;
        ...
        map[dateStr].push({
          ...
          description: r.approval_req_type
            ? `Type: ${r.approval_req_type}`
            : undefined,
    ...
    if (canSeeSchedules) {
      upcomingSchedules.forEach((s) => {
        const dateStr = (s.schedule_due || s.due_date || s.date || "")
          .toString()
          .slice(0, 10);
        if (!dateStr) return;
        map[dateStr] = map[dateStr] || [];
        map[dateStr].push({
          ...
          description:
            ...
            undefined,
          href: s.sched_id
            ? `/tracking/detailed/${s.sched_id}`
            : s.workflow_id
            ? `/financial-overview/detailed/${s.workflow_id}`
            : "/schedule",
          type: "schedule",
        });
      });
    ...
    try {
      ...
      if (canSeeScholarStats) {
        const studentsRes = await axios.get(
          `${VITE_BACKEND_URL}api/maintenance/get-all-students`
        );
      ...
      if (canSeeFinancialStats) {
        const disbursedRes = await axios.get(
          `${VITE_BACKEND_URL}api/disbursement-overview/get-total-disbursed`
        );
          ...
          <main className="flex-1 min-w-0">
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Here's what's happening with your scholarship management system.
              </p>
            </div>
                ...
                {pendingRequests.length === 0 ? (
                  ...
                  <ul className="space-y-2">
                    {pendingRequests.slice(0, 5).map((r) => {
                      const due = (r.approver?.approver_due_date || "")
                        .toString()
                        .slice(0, 10);
                      return (
                      ...
                      if (isMBHR) {
                        ...
                        return (
                          ...
                          >
                            <Link
                              to="/renewal-scholarship"
                              className="text-xs text-gray-700 truncate mr-2 hover:underline"
                            >
