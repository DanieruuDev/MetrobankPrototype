...
const ROIandAnalytics: React.FC = () => {
  ...
  const [selectedPredictionYear, setSelectedPredictionYear] =
    useState<number>(2); // AI Prediction year selector
  ...
  // yearlyValueGenerated is now the total value generated by all absorbed scholars
  const totalYearlyGain = yearlyValueGenerated;
  ...
  const generateROISeries = (years: number) => {
    ...
    for (let i = 1; i <= years; i++) {
      ...
      // Calculate yearly gain based on retention rate
      // Value generated is proportional to the number of scholars retained
      const retentionRate = scholarsRetainedThisYear / actualHiredScholars;
      const yearlyGainThisYear = yearlyValueGenerated * retentionRate;
  ...
  const calculateAIPrediction = (selectedYear: number) => {
    ...
    for (let year = 1; year <= selectedYear; year++) {
      ...
      // Calculate value generated based on retention rate
      const retentionRate = scholarsAtYear / actualHiredScholars;
      const yearlyValueAtYear = yearlyValueGenerated * retentionRate;
      cumulativeValueGenerated += yearlyValueAtYear;
    }
    ...
    // Calculate value based on retention rate
    const retentionRate = scholarsRetainedAtYearX / actualHiredScholars;
    const annualValueAtYearX = yearlyValueGenerated * retentionRate;
          ...
          <div className="max-w-[1900px] mx-auto">
            ...
            <div className="bg-white/70 backdrop-blur-md border border-white/30 rounded-xl shadow-lg p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mx-1 sm:mx-0">
              <div className="flex flex-col">
                ...
                >
                  <span className="hidden sm:inline">
                    Average External Hiring Cost (PHP)
                  </span>
              ...
              <div className="flex flex-col">
                ...
                >
                  <span className="hidden sm:inline">
                    Average Yearly Value Generated by All Absorbed Scholars
                    (PHP)
                  </span>
                  <span className="sm:hidden">Total Yearly Value</span>
                </label>
                <input
                  id="value-input"
                  type="number"
                ...
                />
                <p className="text-xs text-gray-600 mt-1 leading-relaxed">
                  Total annual value generated by ALL absorbed scholars combined
                  (productivity and salary). (Max: ₱1T)
                </p>
              </div>
            </div>
                    ...
                    <div className="bg-green-50 p-4 rounded-lg">
                      ...
                      <div className="text-sm text-gray-700 space-y-1">
                        ...
                        <p>
                          ...
                          {actualHiredScholars}
                          {` (${Math.round(
                            totalStudents * scholarAbsorptionRate
                          )} at ${Math.round(
