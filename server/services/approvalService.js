...
const fetchUnscheduledWorkflows = async () => {
  const query = `
    ...
    FROM
      workflow w
    INNER JOIN wf_request_type_maintenance rt 
      ON w.rq_type_id = rt.rq_type_id 
    INNER JOIN maintenance_semester ms 
      ON w.semester_code = ms.semester_code
    INNER JOIN maintenance_sy sy 
    ...
    WHERE
      ...
      AND NOT EXISTS (
        ...
        FROM event_schedule es
        JOIN disbursement_schedule ds ON ds.sched_id = es.sched_id
        JOIN workflow w2 ON ds.workflow_id = w2.workflow_id
        WHERE es.sy_code = w.sy_code
          ...
          AND (
            -- ✅ For internship, match by covered_date
            (rt.disbursement_type_id = 4 AND w2.covered_date = w.covered_date)
            OR
            -- ✅ For others, ignore covered_date
            (rt.disbursement_type_id <> 4)
          )
      )
    ORDER BY w.sy_code DESC, w.semester_code ASC, w.covered_date ASC;
  `;
